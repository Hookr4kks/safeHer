...
function telaTeste(){
  ...
  if(testeState.finalizado){
    ...
    /* tipos de violência com indícios (qualquer resposta acima de "Nunca" no bloco) */
    const blocos = TESTE_BLOCOS[state.idioma];
    ...
    <div class="card">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-top:20px;">
        ${escala.map((rotulo,valor)=>`
          <button class="opcao-sn ${respostaAtual===valor?'selecionada':''}" style="justify-content:flex-start;padding:12px 16px;" onclick="responderTeste(${valor})">${rotulo}</button>
        `).join("")}
      </div>
    </div>
  </div>`;
